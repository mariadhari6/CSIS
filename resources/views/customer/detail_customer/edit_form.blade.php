...
<td>
    <select class="select CompanyId-{{ $details->id }}" id="CompanyId" disabled>
        ...
        <option value="{{ $item->id }}" {{ $item->id == $details->company_id ? 'selected':'' }}>{{ $item->company_name
            }}</option>
        @endforeach
    </select>
</td>
<td>
    <select class="select LicencePlate-{{ $details->id }}" id="LicencePlate" name="LicencePlate">
        ...
        <option value="{{ $item->id }}" {{ $details->licence_plate == $item->id ? 'selected':'' }}>{{
...
<td>
    <select class="select PoNumber-{{ $details->id }}" id="PoNumber" name="PoNumber">
        @foreach ($po as $item)
        <option value="{{ $item->id }}" {{ $details->po_id == $item->id ? 'selected':'' }}>{{ $item->po_number }}
        </option>
        @endforeach
    </select>
...
<td>
    <select class="select HargaLayanan-{{ $details->id }}" id="HargaLayanan" name="HargaLayanan" disabled>
        @foreach ($po as $item)
        <option value="{{ $item->id }}" {{ $details->harga_layanan == $item->id ? 'selected':'' }}>{{
...
<td>
    <select class="select PoDate-{{ $details->id }}" id="PoDate" name="PoDate" disabled>
        @foreach ($po as $item)
        <option value="{{ $item->id }}" {{ $details->po_date == $item->id ? 'selected':'' }}>{{ $item->po_date }}
        </option>
        @endforeach
    </select>
</td>
<td>
    <select class="select StatusPo-{{ $details->id }}" id="StatusPo" name="StatusPo" disabled>
        @foreach ($po as $item)
        <option value="{{ $item->id }}" {{ $details->status_po == $item->id ? 'selected':'' }}>{{ $item->status_po }}
        </option>
        @endforeach
    </select>
</td>
...
<td>
    <select class="select GSM-{{ $details->id }}" id="GSM" name="GSM">
        ...
        @foreach ($gsm as $item)
        <option value="{{ $item->id }}" {{ $details->gsm_id == $item->id ? 'selected':''}}>{{ $item->gsm_number }}
        </option>
        @endforeach
    </select>
...
<td>
    <select class="select Provider-{{ $details->id }}" id="Provider" name="Provider">
        ...
        @foreach ($gsm as $item)
        <option value="{{ $item->id }}" {{ $details->provider == $item->id ? 'selected':''}}>{{ $item->provider }}
        </option>
        @endforeach
    </select>
</td>
<td>
    <div class="col m" id="sensor">
        <a>
            <option class="SensorAll-{{$details->id}}" value="{{ $details->sensor_all }}" id="SensorAll"
                data-toggle="modal" data-target="#exampleModal">{{ $details->sensor_all }}</option>
        </a>
    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select class="custom-select" id="SensorName" name="SensorName" disabled>
                            @foreach ($sensor as $item)
                            <option value="{{ $item->sensor_name }}" {{ old('sensor_id')==$item->id ? 'selected':''}}>{{
                    ...
                    <div class="input-group">
                        ...
                        <textarea class="form-control" aria-label="With textarea" id="SensorTerpilih"
                            disabled>{{ $details->sensor_all}}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
<td>
    <select class="select PoolName-{{ $details->id }}" id="PoolName" disabled name="PoolName">
        ...
        @foreach ($vehicle as $item)
        <option value="{{ $item->id }}" {{ $details->licence_plate == $item->id ? 'selected':'' }}>{{ $item->pool_name
            }}</option>
        @endforeach
    </select>
</td>
...
<td>
    <div class="input-div"><input type="date" class="input Waranty-{{ $details->id }}" id="Waranty"
            value="{{ $details->waranty }}"></div>
</td>
<td>
    <select class="select StatusLayanan-{{ $details->id }}" id="StatusLayanan" name="status_layanan">
        @foreach ($status_layanan as $item)
        <option value="{{ $item->id }}" {{ $details->status_id == $item->id ? 'selected':'' }}>{{
            $item->service_status_name }}</option>
        @endforeach
    </select>
</td>
<td id="tanggal_aktif">
    <div class="input-div"><input type="date" class="input TanggalPasang-{{ $details->id }}" id="TanggalPasang"
            value="{{ $details->tanggal_pasang }}"></div>
</td>
<td id="tanggal_non_aktif">
    <div class="input-div"><input type="date" class="input TanggalNonAktif-{{ $details->id }}" id="TanggalNonAktif"
            value="{{ $details->tanggal_non_aktif }}" data-toggle="popover" data-placement="bottom"
            data-content="Please fill out the field"></div>
</td>
<td id="tanggal_reaktivasi">
    <div class="input-div"><input type="date" class="input TanggalReaktivasi-{{ $details->id }}" id="TanggalReaktivasi"
            value="{{ $details->tgl_reaktivasi_gps}}" data-toggle="popover" data-placement="bottom"
            data-content="Please fill out the field"></div>
</td>
