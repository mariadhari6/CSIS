<tr id="add_form">
    <td></td>
    <td><i class="fas fa-check add" id="add" onclick="store()"></i><i class="fas fa-times cancel" onclick="cancel()"></i></td>
<<<<<<< HEAD
    <td><div class="input-div"><input type="text" class="input" id="FirstName" placeholder="First Name" ></i></td>

    <td><div class="input-div"><input type="text" class="input" id="LastName" placeholder="Last Name" ></i></td>
=======
<<<<<<< HEAD
    <td><div class="input-div"><input type="text" class="input" id="FirstName" placeholder="First Name" required></i></td>

    <td><div class="input-div"><input type="text" class="input" id="LastName" placeholder="Last Name" required></i></td>
=======
    <td> <div class="input-div"><input type="text" class="input" id="FirstName" placeholder="First Name"></i></div></td>
    <td> <div class="input-div"><input type="text" class="input" id="LastName" placeholder="Last Name"></i></td>
>>>>>>> 16a71c4f897e3f5521f93dffe30c0dfcfddb2131
>>>>>>> 931300e66e6b242e64c71277293e48dba27a7aeb
</tr>
